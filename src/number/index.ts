 ...
 * 加法函数
 */
export function accAdd(num1: number, num2: number, ...rest: number[]): number {
 ...
 * 减法函数
 */
export function accSub(num1: number, num2: number, ...rest: number[]): number {
  // const r1 = (num1.toString().split('.')[1] || '').length;
  // const r2 = (num2.toString().split('.')[1] || '').length;
 ...
 * 乘法函数
 */
export function accMulti(num1: number, num2: number, ...rest: number[]): number {
 ...
 * 除法函数
 */
export function accDiv(num1: number, num2: number, ...rest: number[]): number {
  const r1 = (num1.toString().split('.')[1] || '').length;
  const r2 = (num2.toString().split('.')[1] || '').length;
  ...
  if (rest && rest.length > 0) {
    return accDiv(
      (num1 / num2) * Math.pow(10, r2 - r1),
      rest.shift() as number,
      ...rest,
    );
  }
  return (num1 / num2) * Math.pow(10, r2 - r1);
}
